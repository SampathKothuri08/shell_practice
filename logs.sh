...
then
    ...
    echo -e "$G You have root access to run the script $N" | tee -a $LOG_FILE
fi
...
Validate(){
    ...
    then
        ...
        echo -e "$Y$2 is unavailable on your server, let's install it $N" | tee -a $LOG_FILE
        dnf install $2 -y &>> $LOG_FILE

        if [ $? -ne 0 ]
        then
            echo -e "$R Error: Failed to install $2 $N" | tee -a $LOG_FILE
        else
